...
///U8GLIB_PCD8544 u8g(U8G_I2C_OPT_NONE) ;
...
keyMap encBtn_map[] = {
  {btnEnter, options->getCmdChar(enterCmd)},
  {btnEsc, options->getCmdChar(escCmd)},
  {btnUp, options->getCmdChar(upCmd)},
  {btnDown, options->getCmdChar(downCmd)}
};//negative pin numbers use internal pull-up, this is on when low
keyIn<4> encButton(encBtn_map);//1 is the number of keys

//input from the encoder + encoder button + serial
Stream* inputsList[] = {&encButton, &Serial};
